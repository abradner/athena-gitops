...
| `cluster/apps/` | Workload sync targets: the `pmn` ApplicationSet (per-service, per-environment apps from `pmn-workloads`), repo credentials, the hello-world canary, and the first-party apps (mcg, kaff, spritz) plus the shared Temporal cluster — each owned by its own Application in the `workloads` project. |
